...
int backsockfd, securitySockId, oGatewaysockfd;
int vectorClock[4] = {0};
FILE *fpOutputFile;
char *file1, *file3;	
...
{	
	...
	struct sockaddr_in oGateway;
	//create the socket
	if((oGatewaysockfd = socket(AF_INET, SOCK_STREAM, 0)) == -1)
	{
		printf("\nCould not create oGateway socket");
	}

	//Initialise the oGateway socket
	oGateway.sin_family = AF_INET;
	oGateway.sin_addr.s_addr = inet_addr(pGatewayIP);
	oGateway.sin_port = htons( pGatewayPort );

	//Connect gateway to oGatewaysockfd
	if((connect(oGatewaysockfd, (struct sockaddr *) &oGateway, sizeof(oGateway))) < 0)
	{
		perror("\nUnable to connect Gateway to other Gateway");
		return;
	}
	
	puts("gateway: Connected to oGatewaysockfd");
	
	write(oGatewaysockfd, configInfo, strlen(configInfo), 0);
    puts(configInfo);
	//-------------------------------------------------------------------------------------------------------
